...
//  console.log("Price of ETH/USD", ETHprice);
  const Amm = await upgrades.deployProxy(amm, [baseAsset,baseAsset.mul(weiPrice).div(Default_digits),BigNumber.from(90).mul(Default_digits).div(100), BigNumber.from(3600), deployedAdd.PriceFeedL2Imp, "BTC", "0xA3C957f5119eF3304c69dBB61d878798B3F239D9", BigNumber.from(12).mul(Default_digits).div(10000), BigNumber.from(27).mul(Default_digits).div(100000), BigNumber.from(3).mul(Default_digits).div(1000000)], {initializer : "initialize" });
 await Amm.deployed();

//  const impAddr = getImplementation(Amm.address);
