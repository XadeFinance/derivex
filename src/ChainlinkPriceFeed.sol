...
contract ChainlinkL1 is XadeOwnableUpgrade, BlockContext {
    ...
    event PriceUpdated(bytes32 priceFeedKey, uint256 price, uint256 timestamp);
    ...
    function setPriceFeedL2(address _priceFeedL2) public onlyOwner {
        ...
        priceFeedL2 = IPriceFeed(_priceFeedL2);
    ...
    function updateLatestRoundData(bytes32 _priceFeedKey) external {
        ...
        priceFeedL2.setLatestData(_priceFeedKey, decimalPrice.toUint(), timestamp);
        emit PriceUpdated(_priceFeedKey, decimalPrice.toUint(), timestamp);
